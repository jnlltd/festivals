...
<g:each in="${changes}" var="festival">
    <p style="margin-top: 10px; color: #7F7F7F; font-weight: bold; font-size: 14px;">
        ...
        <festival:show absolute="true" id="${festival.key.id}" name="${festival.key.name}">
            <strong>${festival.key.name}</strong>
        </festival:show>
    </p>

    <g:set var="festivalLineupChange" value="${festival.value}"/>
    <g:render template="/email/festivalChanges" model="[changes: festivalLineupChange.netAddedArtists, festival: festival]"/>

    <g:render template="/email/festivalChanges" model="[changes: festivalLineupChange.netDeletedArtists, festival: festival, removed: true]"/>
</g:each>
...
<div style="margin-top: 15px; font-size: 10px;">
    ...
    <g:link controller="festival" action="subscriptions" absolute="true">clicking here</g:link>
    or copying the following link into your browser's address bar:
    <g:createLink controller="festival" action="subscriptions" absolute="true"/>
</div>