...
searchable {
    ...
    if (Environment.current in [Environment.DEVELOPMENT, Environment.TEST]) {
        compassConnection = new File("${userHome}/.grails/projects/${appName}/searchable-index/${grailsEnv}").absolutePath

    } else {
        assert userHome, "Failed to set searchable index location because home dir environment variable not set"
        compassConnection = "$userHome/festivals-files/searchable-index"
    }
     ...
     */
    defaultExcludedProperties = ["password", "version"]
...
environments {
    ...
    test {
        searchable {
            ...
            // don't index domain classes in test because it screws up the test data builder
            mirrorChanges = false
